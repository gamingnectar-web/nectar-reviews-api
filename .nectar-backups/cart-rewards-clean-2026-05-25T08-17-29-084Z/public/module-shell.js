/*
  ...
  - Keeps one Shopify app identity.
  - Routes the left sidebar to a product/module workspace.
  - Loads module assets from /public/modules/<module>/ when selected.
*/
(function NectarModuleShell() {
  ...
  function getModules() {
    ...
    return modules.length ? modules : [
      { id: REVIEW_MODULE, productSlug: 'review-widget', label: 'review-widget', legacy: true, defaultModule: true },
      { id: CART_MODULE, productSlug: CART_MODULE, label: 'Cart Milestone Rewards', css: '/modules/cart-rewards/admin.css', script: '/modules/cart-rewards/admin.js' }
    ];
  }
  ...
  function getInitialModule() {
    ...
    return normaliseModuleId(sessionStorage.getItem(STORAGE_KEY) || DEFAULT_MODULE);
  }
  ...
  function activateReviewWidget() {
    document.body.dataset.nectarActiveModule = REVIEW_MODULE;
  ...
  function activateCartRewards() {
    document.body.dataset.nectarActiveModule = CART_MODULE;
    if (window.NectarCartRewardsAdmin && typeof window.NectarCartRewardsAdmin.setProductMode === 'function') {
