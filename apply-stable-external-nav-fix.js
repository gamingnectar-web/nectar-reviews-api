...
const navPath = path.join(root, 'admin-nav-stable.js');
...
/**
 * Remove old appended nav controllers from admin.js.
 * Keep the original admin functions intact.
 */
const markers = [
  ...
  '/* HARD RESET fast top tabs + contextual side tabs'
];

let firstMarker = -1;

for (const marker of markers) {
  const idx = adminJs.indexOf(marker);
...
if (firstMarker !== -1) {
  adminJs = adminJs.slice(0, firstMarker).trimEnd() + '\n';
  write(jsPath, adminJs);
  console.log('Removed old appended nav controller from admin.js');
} else {
  console.log('No old appended nav controller found in admin.js');
}

const stableNavJs = String.raw`
(function () {
      ...
      <div class="nr-manual-code-grid">
        ...
        <div class="panel">
          ...
          <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:14px;">
            <button class="secondary-btn" type="button" data-nr-action="copy-manual-code">Copy Code</button>
            <button class="secondary-btn" type="button" data-nr-action="copy-manual-email">Copy Email Copy</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="nr-email-template-grid">
        <div class="panel">
          ...
          <div class="nr-variable-row">
            <button class="nr-variable-chip" type="button" data-nr-variable="{{ discount_code }}">{{ discount_code }}</button>
            <button class="nr-variable-chip" type="button" data-nr-variable="{{ expiry_days }}">{{ expiry_days }}</button>
            <button class="nr-variable-chip" type="button" data-nr-variable="{{ percentage }}">{{ percentage }}</button>
            <button class="nr-variable-chip" type="button" data-nr-variable="{{ customer_email }}">{{ customer_email }}</button>
          </div>
...
It expires in {{ expiry_days }} days and can only be used once.</textarea>
          ...
          <button class="post-btn" type="button" data-nr-action="save-email-template">Save Email Template</button>
        </div>
        ...
        <div class="panel">
          ...
          <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:14px;">
            <button class="secondary-btn" type="button" data-nr-action="refresh-email-preview">Refresh Preview</button>
            <button class="secondary-btn" type="button" data-nr-action="copy-email-preview">Copy Preview</button>
          </div>
        </div>
      </div>
    </div>
  ...
  function apiBase() {
    return typeof window.API !== 'undefined' && window.API ? window.API : 'https://nectar-reviews-api.onrender.com/api';
  }

  function shopDomain() {
    if (typeof window.SHOP_DOMAIN !== 'undefined' && window.SHOP_DOMAIN) return window.SHOP_DOMAIN;
  ...
  function findSidebarGroup() {
    ...
    if (groups.length) {
      const configuration = groups.find(group => /configuration/i.test(group.textContent || ''));
      const developers = groups.find(group => /developers/i.test(group.textContent || ''));
      return groups.find(group => group !== configuration && group !== developers) || groups[0];
    }
  ...
  function sideButton(label, module, sub) {
    ...
    return `
      <button type="button" class="nr-side-tab${active}" data-nr-module="${module}" data-nr-sub="${sub}">
        ${label}
      </button>
    `;
  }
  ...
  function ensureDiscountExtras() {
    ...
    if (!q('reward-manual')) {
      ...
      if (settings) settings.insertAdjacentHTML('afterend', discountExtrasHtml);
      else discountView.insertAdjacentHTML('beforeend', discountExtrasHtml);
    }
  ...
  function showDiscountSubView(sub) {
    ...
    if (sub === 'codes' && !state.rewardCodesLoaded) {
      ...
      setTimeout(loadRewardCodes, 0);
    }

    if (sub === 'email') {
      setTimeout(renderRewardEmailPreview, 0);
    }
  }
  ...
  function select(module, sub) {
    ...
    if (state.module === 'discounts') {
      ...
      if (!state.rewardSettingsLoaded) {
        ...
        setTimeout(loadRewardSettings, 0);
      }
    }
  ...
  function insertRewardVariable(variable) {
    const box = getTemplateBox();
    if (!box) return;
  ...
  document.addEventListener('click', event => {
    const navButton = event.target.closest('[data-nr-module][data-nr-sub]');
    if (navButton) {
      event.preventDefault();
      event.stopPropagation();
      select(navButton.dataset.nrModule, navButton.dataset.nrSub);
      return;
    }

    const variableButton = event.target.closest('[data-nr-variable]');
    if (variableButton) {
      event.preventDefault();
      insertRewardVariable(variableButton.dataset.nrVariable);
      return;
    }

    const action = event.target.closest('[data-nr-action]')?.dataset.nrAction;
    if (!action) return;

    event.preventDefault();

    if (action === 'create-manual-code') createManualRewardCode();
    if (action === 'save-email-template') saveRewardSettings();
    if (action === 'refresh-email-preview') renderRewardEmailPreview();
    if (action === 'copy-email-preview') copyText(q('reward-email-preview')?.textContent, 'Email preview copied');
    if (action === 'copy-manual-code') copyText(state.lastManualReward?.code, 'Code copied');
    if (action === 'copy-manual-email') copyText(q('manual-reward-email-copy')?.textContent, 'Email copy copied');
  }, true);

  window.loadRewardSettings = loadRewardSettings;
  window.saveRewardSettings = saveRewardSettings;
  window.loadRewardCodes = loadRewardCodes;
  window.renderRewardEmailPreview = renderRewardEmailPreview;

  function detectInitialState() {
    const activeView = document.querySelector('.view.active');

    if (activeView && activeView.id === 'v-mgr') {
      state.module = 'reviews';
      state.sub = 'manager';
      return;
    }

    if (activeView && activeView.id === 'v-discounts') {
...
fs.writeFileSync(navPath, stableNavJs);
console.log('Created admin-nav-stable.js');

/**
 * Add stable nav script after admin.js.
 */
html = html.replace(/<script[^>]+admin-nav-stable\.js[^>]*><\/script>\s*/g, '');

const stableScript = '<script src="/admin-nav-stable.js?v=stable-1"></script>';

if (html.includes('</body>')) {
  html = html.replace('</body>', `  ${stableScript}\n</body>`);
} else {
  html += `\n${stableScript}\n`;
}
...
console.log('Done. Stable external navigation installed.');
