...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    if (sorted.length) {
      ...
      try {
        const productData = await shopifyFetch(`/admin/api/2024-01/products/${id}.json?fields=id,title,image`, { shopDomain });
...
router.get('/products/search', async (req, res, next) => {
  try {
    ...
    const data = await shopifyFetch('/admin/api/2024-01/products.json?limit=250&fields=id,title,handle,image,variants,tags', { shopDomain: shopDomainFromReq(req) });
