...
router.get('/shopify', (req, res) => {
  ...
  if (!shopDomain || !isValidShopDomain(shopDomain)) {
    return res.status(400).send('Valid Shopify shop domain is required.');
  }
...
async function callbackHandler(req, res, next) {
  try {
    ...
    if (!shopDomain || !isValidShopDomain(shopDomain)) {
      return res.status(400).send('Invalid shop in OAuth callback.');
    }
    if (!code) {
    ...
    if (!verifyState(state, shopDomain)) {
      return res.status(400).send('Invalid or expired OAuth state. Please install again.');
    }
