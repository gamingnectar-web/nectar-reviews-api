...
function cleanConditionalRules(list = []) {
  return (Array.isArray(list) ? list : []).slice(0, 80).map((rule) => ({
    ...
    actionType: ['add_tag', 'recommend_tag', 'set_product_type', 'set_vendor', 'set_metafield', 'set_theme_template', 'add_collection', 'title_prefix', 'title_suffix'].includes(rule.actionType) ? rule.actionType : 'recommend_tag',
...
function applyConditionalRules(draft = {}, settings = {}) {
  const next = { ...draft, tags: parseTags(draft.tags), recommendedTags: parseTags(draft.recommendedTags), collections: Array.isArray(draft.collections) ? draft.collections : parseTags(draft.collections), metafields: normaliseMetafields(draft.metafields || []) };
  for (const rule of settings.conditionalRules || []) {
    ...
    if ((rule.actionType === 'add_tag' || rule.actionType === 'recommend_tag') && rule.actionValue) {
      // Tags require merchant click-approval in the UI. Conditional rules only recommend them.
      next.recommendedTags = Array.from(new Set([...(next.recommendedTags || []), rule.actionValue]));
    ...
    } else if (rule.actionType === 'set_theme_template' && rule.actionValue) {
      next.themeTemplate = rule.actionValue;
    } else if (rule.actionType === 'add_collection' && rule.actionValue) {
      next.collections = Array.from(new Set([...(next.collections || []), rule.actionValue]));
