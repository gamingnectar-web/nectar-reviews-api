...
function normaliseDraftProduct(raw = {}) {
  ...
  // Tags are intentionally merchant-approved only. The importer can suggest tags,
  // but it should not add product-import/url-import/invoice-import automatically.
  const tags = Array.from(new Set(parseTags(raw.tags))).slice(0, 40);
  ...
  return {
    ...
    productCategory: cleanText(raw.productCategory || raw.shopifyCategory || raw.taxonomyCategory || '', 180),
    themeTemplate: cleanText(raw.themeTemplate || raw.templateSuffix || raw.template_suffix || '', 80).replace(/^(product\.)/i, ''),
    collections: Array.from(new Set([...(Array.isArray(raw.collections) ? raw.collections : parseTags(raw.collections)), ...(Array.isArray(raw.collectionHandles) ? raw.collectionHandles : parseTags(raw.collectionHandles))])).slice(0, 40),
    recommendedTags: Array.from(new Set(parseTags(raw.recommendedTags))).slice(0, 40),
    status: 'draft',
    tags,
    price,
    cost,
    ...
    barcodeStatus: cleanText(raw.barcodeStatus || (raw.barcode || raw.gtin || raw.gtin13 ? 'provided' : 'needs_check'), 30),
    harmonizedSystemCode: cleanText(raw.harmonizedSystemCode || raw.hsCode || raw.harmonisedSystemCode || '', 32),
    dietarySupplement: cleanText(raw.dietarySupplement || '', 20),
    packageType: cleanText(raw.packageType || '', 80),
    beverageProductForm: cleanText(raw.beverageProductForm || '', 80),
    salesChannelPolicy: cleanText(raw.salesChannelPolicy || 'all', 30),
    compareAtPriceMode: cleanText(raw.compareAtPriceMode || (raw.compareAtPrice || raw.originalPrice ? 'value' : 'needs_check'), 30),
    fieldConfirmations: raw.fieldConfirmations && typeof raw.fieldConfirmations === 'object' ? raw.fieldConfirmations : {},
    fieldInference: raw.fieldInference && typeof raw.fieldInference === 'object' ? raw.fieldInference : {},
    weight,
    weightUnit,
