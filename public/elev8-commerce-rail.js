(function Elev8CommerceRail(){
  ...
  async function api(){
    ...
    const shop=new URLSearchParams(location.search).get('shop')||new URLSearchParams(location.search).get('shopDomain')||'';
    const q=shop?`?shopDomain=${encodeURIComponent(shop)}`:'';
    const res=await fetch(`/api/admin/elev8-commerce/commerce-pulse${q}`,{credentials:'same-origin'});
  ...
  function ensureRail(){
    ...
    if(rail)return rail;
    rail=document.createElement('div');rail.id='e8-commerce-rail';
  ...
  function render(data){
    ...
    const p=data?.periods||{},today=p.today||{},week=p.week||{},month=p.month||{},six=p.sixMonths||{};
    rail.innerHTML=`
      <div class="e8-pulse-block"><span class="e8-pulse-kicker">Today</span><strong>${fmtMoney(today.sales)}</strong><small>${fmtNum(today.orders)} orders · ${fmtMoney(today.aov)} AOV</small></div>
      <div class="e8-pulse-grid"><div><span>7 days</span><strong>${fmtMoney(week.sales)}</strong></div><div><span>30 days</span><strong>${fmtMoney(month.sales)}</strong></div></div>
      <div class="e8-pulse-section"><span class="e8-pulse-kicker">Customers · 30d</span><div class="e8-pulse-row"><span>Customers</span><b>${fmtNum(month.customers)}</b></div><div class="e8-pulse-row"><span>Returning</span><b>${fmtPct(month.returningCustomerRate)}</b></div><div class="e8-pulse-row"><span>Return revenue</span><b>${fmtPct(month.returningRevenueRate)}</b></div></div>
      <div class="e8-pulse-section"><span class="e8-pulse-kicker">Profitability · 30d</span><div class="e8-pulse-row"><span>Gross profit</span><b>${fmtMoney(month.grossProfit)}</b></div><div class="e8-pulse-row"><span>Margin</span><b>${fmtPct(month.grossMargin)}</b></div><div class="e8-pulse-row"><span>Profit / order</span><b>${fmtMoney(month.grossProfitPerOrder)}</b></div><div class="e8-pulse-row"><span>Cost coverage</span><b>${fmtPct(month.costCoverage)}</b></div></div>
      <div class="e8-pulse-section e8-pulse-muted"><span>6m sales</span><b>${fmtMoney(six.sales)}</b></div>`;
  }
  ...
  function style(){
    ...
    s.textContent=`
      ...
      #e8-commerce-rail{display:none}body.e8-context-home #e8-commerce-rail{display:block}
      .e8-pulse-loading{font-size:12px;color:#7b8493;padding:10px 2px}
      .e8-pulse-block,.e8-pulse-section{padding:13px 0;border-top:1px solid #e6eaf0}.e8-pulse-block{border-top:0;padding-top:0}
      .e8-pulse-block>strong{display:block;font-size:25px;letter-spacing:-.04em;margin:3px 0}.e8-pulse-block small{color:#6b7280;line-height:1.4}
