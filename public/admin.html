...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .import-shell { display:grid; gap:22px; }
    .import-step { border:1px solid var(--border); border-radius:16px; background:#fff; padding:22px; box-shadow:var(--shadow); }
    .import-step h3 { margin:0 0 10px; font-size:20px; letter-spacing:-.02em; }
    .import-row { display:flex; flex-wrap:wrap; gap:12px; align-items:center; margin-top:14px; }
    .import-help { margin:12px 0 0; padding:12px 14px; border-radius:12px; background:#f8fafc; border:1px solid #e4eaf2; color:#475467; line-height:1.5; }
    .import-upload { display:flex; flex-wrap:wrap; gap:14px; align-items:center; padding:18px; border:1px dashed #c7d2e0; border-radius:14px; background:#fbfdff; }
    .import-upload input[type=file] { min-height:44px; }
    .mapper-grid { display:grid; grid-template-columns:repeat(2,minmax(220px,1fr)); gap:14px; }
    .mapper-card { border:1px solid var(--border); border-radius:12px; padding:14px; background:#fbfdff; }
    .mapper-card label { display:block; margin-bottom:8px; font-weight:900; }
    .import-table { width:100%; border-collapse:separate; border-spacing:0; overflow:hidden; border:1px solid var(--border); border-radius:12px; background:#fff; }
    .import-table th,.import-table td { padding:12px; border-bottom:1px solid var(--border); text-align:left; vertical-align:top; }
    .import-table th { background:#f8fafc; font-size:12px; text-transform:uppercase; letter-spacing:.04em; color:#667085; }
...
<body>
      ...
      <section id="v-import" class="view">
        ...
        <div class="import-shell">
          <div class="import-step">
            ...
            <p class="muted">Choose the source format, then upload the CSV. Nothing goes live until you review the staging table.</p>
            <div class="import-row">
              <label for="import-platform-select" style="font-weight:900;">Platform</label>
            ...
            <div id="inst-generic" class="import-inst-box active"><p class="import-help">Generic CSV: include columns for Product ID, Star Rating, Name, Email, Review Title, Review Body, and Date where available.</p></div>
            <div id="inst-weebly" class="import-inst-box"><p class="import-help">Weebly / Square: upload the export and map product IDs in the staging step before publishing.</p></div>
            <div id="inst-judgeme" class="import-inst-box"><p class="import-help">Judge.me: export reviews from Judge.me, upload the CSV, and map the columns below.</p></div>
          </div>
          <div class="import-step">
            <h3>2. Upload and map columns</h3>
            <div class="import-upload">
              <input id="csv-file" type="file" accept=".csv" onchange="window.handleFileUpload()" />
              <span id="file-name" class="muted">No CSV selected yet.</span>
            </div>
            <div id="mapping-ui" style="display:none; margin-top:24px;">
              <h4 style="margin:0 0 14px;">Map your columns</h4>
              <div id="column-mappers" class="mapper-grid"></div>
              <p style="margin-top:18px;"><button id="import-submit-btn" class="primary-btn" onclick="window.generateStagingArea()">Preview &amp; Map Products</button></p>
            </div>
          </div>
        </div>
      </section>
  ...
  <script src="/admin.js?v=secure-5"></script>
  <script src="/admin-review-manager-enhancements.js?v=secure-5"></script>
  <script src="/nectar-score-notch-fix.js?v=secure-5"></script>
  <script src="/admin-dashboard-analytics.js?v=secure-5"></script>
  <script src="/admin-email-tracking-enhancer.js?v=secure-5"></script>
  <script src="/admin-messaging-campaigns.js?v=secure-5"></script>
