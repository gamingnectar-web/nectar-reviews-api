(function NectarCartRewardsAdmin() {
  ...
  const state = {
    ...
    plannerDate: new Date(new Date().getFullYear(), new Date().getMonth(), 1),
    loading: false,
    hydrated: false,
    ...
    view.innerHTML = `
      <div class="ncr-shell">
        ...
        <div class="ncr-panel" data-ncr-panel-view="planner">
          <div class="ncr-card ncr-calendar-card">
            <div class="ncr-card__header">
              <div>
              ...
              <div class="ncr-actions-row">
                ...
                <button class="ncr-btn" type="button" data-ncr-action="planner-today">Today</button>
                <button class="ncr-btn" type="button" data-ncr-action="planner-next">Next</button>
                <button class="ncr-btn ncr-btn--primary" type="button" data-ncr-action="planner-create">Create campaign</button>
              </div>
            </div>
            <div class="ncr-calendar-toolbar">
              <strong id="ncr-calendar-title">Calendar</strong>
              <span class="ncr-muted">Click a day to allocate a new promotion start date.</span>
            </div>
            <div id="ncr-planner-list"></div>
          </div>
        </div>
  ...
  function showPanel(panel) {
    ...
    if (state.panel === 'planner') loadPlanner();
  }
  ...
  async function loadPlanner() {
    try {
      const year = state.plannerDate.getFullYear();
      const month = state.plannerDate.getMonth() + 1;
  ...
  function renderPlanner() {
    ...
    const title = qs('#ncr-calendar-title');
    if (!target) return;

    const date = state.plannerDate || new Date();
    ...
    for (const event of state.plannerEvents || []) {
      const raw = event.startsAt || event.start || event.date || event.endsAt;
      if (!raw) continue;
    ...
    for (let i = 0; i < 42; i += 1) {
      ...
      const key = calendarKey(cell);
      const inMonth = cell.getMonth() === month;
      const events = eventsByDay.get(key) || [];
      cells.push(`
        <button type="button" class="ncr-calendar-day ${inMonth ? '' : 'is-muted'} ${events.length ? 'has-events' : ''}" data-date="${key}">
          <span class="ncr-calendar-day__number">${cell.getDate()}</span>
          <span class="ncr-calendar-day__events">
            ${events.slice(0, 3).map((event) => `<em>${escapeHtml(event.title || event.name || 'Promotion')}</em>`).join('')}
    ...
    target.innerHTML = `
      <div class="ncr-calendar">
        <div class="ncr-calendar__weekdays">${['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((d) => `<span>${d}</span>`).join('')}</div>
        <div class="ncr-calendar__grid">${cells.join('')}</div>
      </div>
      <div class="ncr-calendar-agenda">
        <h3>${escapeHtml(monthName)} plan</h3>
        ${(state.plannerEvents || []).length ? (state.plannerEvents || []).map((event) => `
        ...
        `).join('') : '<div class="ncr-empty ncr-empty--small">No scheduled promotions this month. Click a date to create one.</div>'}
      </div>
    `;

    qsa('.ncr-calendar-day', target).forEach((button) => {
      button.addEventListener('click', () => openNewCampaignModal(button.dataset.date));
    });
  }
  ...
  function renderBuilder() {
    ...
    root.innerHTML = `
      ...
      <div class="ncr-builder-section">
        <div class="ncr-card__header ncr-card__header--tight">
          <div>
            <span class="ncr-section-label">3. Reward product picker</span>
            <p>Search Shopify products, open the native Shopify picker, or paste a variant GID. Rewards are added to the selected tier.</p>
          </div>
          <button type="button" class="ncr-btn" data-builder-action="open-shopify-picker">Open Shopify picker</button>
        </div>
        <div class="ncr-product-picker">
          <div class="ncr-product-picker__target">Adding to: <strong>${escapeHtml((b.tiers[state.activeTierIndex] || {}).title || `Tier ${state.activeTierIndex + 1}`)}</strong></div>
          <div class="ncr-field ncr-field--inline">
            <input class="ncr-input" id="ncr-product-search" placeholder="Search product title, SKU or barcode" autocomplete="off">
            <button type="button" class="ncr-btn" data-builder-action="search-products">Search catalogue</button>
          </div>
          <div class="ncr-manual-picker">
            <label class="ncr-field">Manual reward title<input class="ncr-input" id="ncr-manual-reward-title" placeholder="Free shaker"></label>
            <label class="ncr-field">Variant GID<input class="ncr-input" id="ncr-manual-variant-id" placeholder="gid://shopify/ProductVariant/123456789"></label>
            <button type="button" class="ncr-btn" data-builder-action="add-manual-reward">Add manual reward</button>
          </div>
          ${state.productSearchWarning ? `<div class="ncr-warning">${escapeHtml(state.productSearchWarning)}</div>` : ''}
  ...
  function bindBuilderEvents(root) {
    ...
    const search = qs('#ncr-product-search', root);
    if (search) {
  ...
  function renderProductResults() {
    if (!state.products.length) {
      return '<div class="ncr-empty ncr-empty--small">Search the catalogue, use the native Shopify picker, or paste a variant GID. Selected products are added to the highlighted tier.</div>';
    }
    return state.products.map((product) => `
      <div class="ncr-product-result">
        ...
        <div>
          ...
          <span class="ncr-muted">${escapeHtml(product.vendor || product.status || '')}</span>
          <div class="ncr-product-variants">
            ${(product.variants || []).map((variant) => `
              <button type="button" class="ncr-variant-pill ${variant.availableForSale === false ? 'is-sold-out' : ''}" data-builder-action="add-product-reward"
                ...
                <span>${escapeHtml(variant.title || 'Default')}</span>
                <small>${variant.sku ? escapeHtml(variant.sku) + ' · ' : ''}${variant.availableForSale === false ? 'sold out' : `stock ${escapeHtml(variant.inventoryQuantity ?? 'n/a')}`}</small>
              </button>
            `).join('') || '<span class="ncr-muted">No variants returned.</span>'}
          </div>
        </div>
      </div>
    `).join('');
  ...
  async function handleBuilderAction(action, button) {
    ...
    if (action === 'open-shopify-picker') return openShopifyPicker();

    if (action === 'add-manual-reward') return addManualReward();

    if (action === 'add-product-reward') {
      const added = addRewardToActiveTier({
      ...
      if (added) toast('Reward product added to selected tier.', 'success');
      return renderBuilder();
    }
  }
  ...
  async function searchProducts() {
    ...
    state.productSearchWarning = '';
    if (resultRoot) resultRoot.innerHTML = '<div class="ncr-empty ncr-empty--small">Searching Shopify products…</div>';
    try {
      const json = await api(`/products/search?q=${encodeURIComponent(query)}&first=24`);
      state.products = json.products || [];
      state.productSearchWarning = json.warning || '';
      if (!state.products.length && !state.productSearchWarning) {
        state.productSearchWarning = 'No products matched. Try SKU, product title, or use the native Shopify picker.';
      }
      if (resultRoot) {
      ...
      state.productSearchWarning = error.message || 'Could not search products.';
      if (resultRoot) resultRoot.innerHTML = `<div class="ncr-empty ncr-empty--small">${escapeHtml(state.productSearchWarning)}</div>`;
      toast(state.productSearchWarning, 'error');
    }
  }

  async function openShopifyPicker() {
    if (!window.shopify || typeof window.shopify.resourcePicker !== 'function') {
      toast('Shopify picker is not available in this embedded view. Use catalogue search or manual variant GID.', 'warning');
      return;
    }
    try {
      ...
      for (const product of items) {
        const variants = Array.isArray(product.variants) && product.variants.length ? product.variants : [{ id: product.id, title: 'Default' }];
        for (const variant of variants) {
          if (addRewardToActiveTier({
            productId: product.id,
            variantId: variant.id,
            handle: product.handle || '',
            title: product.title || 'Reward product',
            variantTitle: variant.title || 'Default',
            imageUrl: variant.image?.url || product.images?.[0]?.url || product.featuredImage?.url || '',
            availableForSale: variant.availableForSale !== false
          })) added += 1;
        }
      }
      if (added) {
        toast(`${added} reward variant${added === 1 ? '' : 's'} added to the selected tier.`, 'success');
        renderBuilder();
      }
    } catch (error) {
      if (!/cancel/i.test(String(error?.message || ''))) toast(error.message || 'Shopify picker failed.', 'error');
    }
  }

  function addManualReward() {
    const title = qs('#ncr-manual-reward-title')?.value || 'Manual reward';
    const variantId = qs('#ncr-manual-variant-id')?.value || '';
    if (!variantId || !variantId.includes('ProductVariant')) {
      toast('Paste a Shopify ProductVariant GID first.', 'warning');
      return;
    }
    addRewardToActiveTier({ title, variantTitle: 'Manual variant', productId: '', variantId, imageUrl: '', availableForSale: true });
    toast('Manual reward added to selected tier.', 'success');
    renderBuilder();
  }

  function addRewardToActiveTier(reward) {
    const tier = state.builder.tiers[state.activeTierIndex];
    if (!tier) {
      toast('Select a tier first.', 'warning');
      return false;
    }
    tier.rewards = tier.rewards || [];
    const exists = tier.rewards.some((item) => String(item.variantId) === String(reward.variantId));
    if (exists) {
      toast('That variant is already attached to this tier.', 'warning');
      return false;
    }
    tier.rewards.push({
      ...
      title: reward.title || reward.productTitle || 'Reward product',
      variantTitle: reward.variantTitle || 'Default',
      imageUrl: reward.imageUrl || '',
      quantity: 1,
      discountType: 'free',
      discountValue: 100,
      outOfStockBehaviour: 'hide',
      availableForSale: reward.availableForSale !== false
    });
    return true;
  }
  ...
  function openNewCampaignModal(prefillDate = '') {
    openModal({
      ...
      onMount: (overlay) => {
        qs('#ncr-modal-create-campaign', overlay).addEventListener('click', () => {
          ...
          if (prefillDate) {
            builder.startsAt = `${prefillDate}T09:00`;
            builder.endsAt = `${prefillDate}T23:59`;
  ...
  async function handleAction(action, source) {
    ...
    if (action === 'planner-create') return openNewCampaignModal(calendarKey(state.plannerDate || new Date()));
    if (action === 'planner-prev') { state.plannerDate = new Date(state.plannerDate.getFullYear(), state.plannerDate.getMonth() - 1, 1); return loadPlanner(); }
    if (action === 'planner-next') { state.plannerDate = new Date(state.plannerDate.getFullYear(), state.plannerDate.getMonth() + 1, 1); return loadPlanner(); }
    if (action === 'planner-today') { const now = new Date(); state.plannerDate = new Date(now.getFullYear(), now.getMonth(), 1); return loadPlanner(); }
