...
(function () {
  ...
  function friendlyShopName() {
    const globalName = window.NectarShopName || window.SHOP_NAME || window.shopify?.config?.shopName || window.Shopify?.shopName || '';
    if (globalName && !/\.myshopify\.com$/i.test(String(globalName))) return String(globalName).trim();
    const host = getShopDomain().replace(/\.myshopify\.com$/i, '').replace(/[-_]+/g, ' ').trim();
  ...
  function smtpHostClientError(host = '') {
    const raw = String(host || '').trim();
    if (!raw) return 'SMTP host is required. Use a server name like smtp.gmail.com or smtp.office365.com.';
    if (raw.includes('@')) return 'SMTP host cannot be an email address. Use smtp.gmail.com, smtp.office365.com, or your provider’s SMTP server. Put your email address in SMTP username / From email.';
    if (/^https?:\/\//i.test(raw)) return 'SMTP host should be a host name only, not a URL. Example: smtp.gmail.com.';
    if (!raw.includes('.') && raw !== 'localhost') return 'SMTP host looks incomplete. Example: smtp.gmail.com.';
    return '';
  }

  function applyProviderPreset(force = false) {
    const provider = val('msg-smtp-provider', 'smtp');
    const host = el('msg-smtp-host');
    const port = el('msg-smtp-port');
    const secure = el('msg-smtp-secure');
    if (!host || (!force && host.value && !host.value.includes('@'))) return;
    if (provider === 'gmail') { host.value = 'smtp.gmail.com'; if (port) port.value = '587'; if (secure) secure.value = 'starttls'; }
    if (provider === 'outlook') { host.value = 'smtp.office365.com'; if (port) port.value = '587'; if (secure) secure.value = 'starttls'; }
  }

  function refreshSmtpHostHint() {
    const host = val('msg-smtp-host');
    const target = el('msg-smtp-host-hint');
    const err = smtpHostClientError(host);
    if (target) {
      target.className = `msg-smtp-host-hint ${err ? 'bad' : 'ok'}`;
      target.textContent = err || 'SMTP host looks valid.';
    }
    return err;
  }
  ...
  function ensureColorChoiceModal() {
    ...
    modal.innerHTML = `<div class="msg-color-modal-card" role="dialog" aria-modal="true"><button type="button" class="msg-color-close">×</button><span class="msg-modal-kicker">Email colour</span><h3>Choose colour</h3><p>Use the picker or a preset. Choose <strong>None / transparent</strong> for no background or border.</p><div class="msg-colour-preview-row"><span class="msg-colour-preview-swatch" id="msg-color-preview-swatch"></span><div><strong id="msg-color-preview-label">Colour selected</strong><small id="msg-color-preview-sub">Preview of the selected colour</small></div></div><label>Colour picker</label><input id="msg-color-native" type="color" value="#f8fafc"><div class="msg-color-swatches"><button data-color-value="#f8fafc" style="background:#f8fafc" title="Soft grey"></button><button data-color-value="#fff7ed" style="background:#fff7ed" title="Warm cream"></button><button data-color-value="#ecfdf3" style="background:#ecfdf3" title="Soft green"></button><button data-color-value="#eff6ff" style="background:#eff6ff" title="Soft blue"></button><button data-color-value="#111827" style="background:#111827" title="Dark navy"></button></div><details class="msg-advanced-colour"><summary>Advanced hex value</summary><input id="msg-color-hex" type="text" placeholder="#f8fafc or none"></details><div class="msg-actions msg-color-actions"><button type="button" class="msg-btn secondary" data-color-none>None / transparent</button><button type="button" class="msg-btn" data-color-apply>Apply colour</button></div></div>`;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* Final email-builder polish: proper two-column builder, premium cards and compact right preview */
      #nr-messaging-campaigns-mount{max-width:1680px;margin:0 auto;}
      #msg-pane-builder .msg-builder-grid{display:grid!important;grid-template-columns:minmax(640px,1fr) clamp(340px,24vw,430px)!important;gap:20px!important;align-items:start!important;}
      #msg-pane-builder .msg-preview-column{position:sticky!important;top:18px!important;align-self:start!important;min-width:0!important;}
      #msg-pane-builder .msg-preview-card{border:1px solid #d9e0ea!important;border-radius:20px!important;box-shadow:0 18px 42px rgba(15,23,42,.08)!important;background:#fff!important;}
      #msg-pane-builder .msg-preview-stage{min-height:540px!important;max-height:calc(100dvh - 160px)!important;overflow:auto!important;padding:18px!important;background:radial-gradient(circle at top,#fff 0,#f7f9fc 42%,#eef2f7 100%)!important;}
      #msg-pane-builder .msg-preview-wrap{transform-origin:top center;}
      .msg-card{border-color:#dce3ec!important;border-radius:18px!important;box-shadow:0 8px 24px rgba(15,23,42,.045)!important;}
      .msg-card h3{font-size:17px!important;letter-spacing:-.02em!important;}
      .msg-card input,.msg-card select,.msg-card textarea{border-radius:12px!important;border-color:#cfd8e3!important;background:#fff!important;}
      .msg-btn,.primary-btn,.secondary-btn{border-radius:13px!important;font-weight:900!important;letter-spacing:-.01em!important;}
      .msg-btn.secondary{background:#fff!important;border:1px solid #cfd8e3!important;color:#101828!important;}
      .msg-template-list{gap:12px!important;}
      .msg-template-card{display:grid!important;gap:14px!important;border:1px solid #dce3ec!important;background:linear-gradient(180deg,#fff,#fbfcfe)!important;border-radius:18px!important;padding:16px!important;}
      .msg-template-top{display:flex!important;align-items:flex-start!important;justify-content:space-between!important;gap:14px!important;}
      .msg-template-top strong{font-size:16px!important;letter-spacing:-.02em!important;}
      .msg-template-meta{display:flex!important;flex-wrap:wrap!important;gap:6px!important;margin-top:8px!important;}
      .msg-template-meta span{font-size:12px!important;border-radius:999px!important;padding:4px 9px!important;background:#f3f6fa!important;border:1px solid #e1e7ef!important;color:#475467!important;}
      .msg-template-actions{display:flex!important;flex-wrap:wrap!important;gap:8px!important;margin:0!important;}
      .msg-template-remove,.msg-icon-btn.danger{width:34px!important;height:34px!important;border-radius:999px!important;background:#fff1f3!important;color:#b42318!important;border:1px solid #fecdd6!important;}
      .msg-color-modal{background:rgba(15,23,42,.54)!important;backdrop-filter:blur(8px)!important;}
      .msg-color-modal-card{width:min(520px,calc(100vw - 32px))!important;border-radius:28px!important;border:1px solid rgba(255,255,255,.55)!important;padding:28px!important;box-shadow:0 40px 120px rgba(15,23,42,.38)!important;}
      .msg-modal-kicker{display:inline-flex;border-radius:999px;background:#eef2ff;color:#3730a3;padding:5px 9px;font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px;}
      .msg-colour-preview-row{display:grid!important;grid-template-columns:auto 1fr!important;align-items:center!important;gap:12px!important;background:#f8fafc!important;border:1px solid #e1e7ef!important;border-radius:18px!important;padding:14px!important;}
      .msg-colour-preview-row small{display:block;color:#667085;font-size:12px;margin-top:2px;}
      .msg-colour-preview-swatch{width:34px!important;height:34px!important;border-radius:999px!important;}
      .msg-color-modal-card input[type=color]{height:56px!important;border-radius:16px!important;padding:6px!important;}
      .msg-color-swatches{gap:10px!important;margin:16px 0!important;}
      .msg-color-swatches button{width:46px!important;height:46px!important;}
      .msg-advanced-colour{border:1px solid #e1e7ef;border-radius:14px;padding:10px 12px;background:#fbfcfe;margin:12px 0;}
      .msg-advanced-colour summary{cursor:pointer;font-weight:900;color:#475467;}
      .msg-advanced-colour input{margin-top:10px!important;}
      .msg-color-actions{justify-content:flex-end!important;}
      .msg-smtp-host-hint{margin:8px 0 0;border-radius:12px;padding:10px 12px;background:#f8fafc;border:1px solid #e1e7ef;color:#475467;font-size:13px;font-weight:800;}
      .msg-smtp-host-hint.bad{background:#fff1f3;border-color:#fecdd6;color:#b42318;}.msg-smtp-host-hint.ok{background:#ecfdf3;border-color:#abefc6;color:#027a48;}
      .smtp-host-action{grid-template-columns:minmax(0,1fr) auto!important;}
      @media(max-width:1180px){#msg-pane-builder .msg-builder-grid{grid-template-columns:1fr!important;}#msg-pane-builder .msg-preview-column{position:static!important;}#msg-pane-builder .msg-preview-stage{max-height:none!important;}}
  ...
  function renderEmailSections(){
      ...
      <div class="msg-section-row" draggable="true" data-section-index="${i}">
        ...
        <div class="msg-section-editor">
          ...
          <div class="msg-section-style-grid">
            ...
            <label>Width snap<select data-section-index="${i}" data-section-field="widthPct"><option value="20" ${Number(item.widthPct || 100) === 20 ? 'selected' : ''}>20%</option><option value="40" ${Number(item.widthPct || 100) === 40 ? 'selected' : ''}>40%</option><option value="60" ${Number(item.widthPct || 100) === 60 ? 'selected' : ''}>60%</option><option value="80" ${Number(item.widthPct || 100) === 80 ? 'selected' : ''}>80%</option><option value="100" ${Number(item.widthPct || 100) === 100 ? 'selected' : ''}>100%</option></select></label>
            <label>Height snap<select data-section-index="${i}" data-section-field="heightPct"><option value="5" ${Number(item.heightPct || 5) === 5 ? 'selected' : ''}>5%</option><option value="10" ${Number(item.heightPct || 5) === 10 ? 'selected' : ''}>10%</option><option value="15" ${Number(item.heightPct || 5) === 15 ? 'selected' : ''}>15%</option><option value="20" ${Number(item.heightPct || 5) === 20 ? 'selected' : ''}>20%</option><option value="30" ${Number(item.heightPct || 5) === 30 ? 'selected' : ''}>30%</option><option value="40" ${Number(item.heightPct || 5) === 40 ? 'selected' : ''}>40%</option></select></label>
          </div>
          <div class="msg-section-snap-actions"><button type="button" class="msg-btn secondary" data-section-nudge="${i}:width:-20">Narrower</button><button type="button" class="msg-btn secondary" data-section-nudge="${i}:width:20">Wider</button><button type="button" class="msg-btn secondary" data-section-nudge="${i}:height:5">Taller</button><button type="button" class="msg-btn secondary" data-section-nudge="${i}:height:-5">Shorter</button></div>
    ...
    box.querySelectorAll('[data-section-field]').forEach((input)=>['input','change'].forEach((eventName)=>input.addEventListener(eventName,()=>{
      const i=Number(input.dataset.sectionIndex); if(!emailSections[i]) return;
      const field=input.dataset.sectionField; emailSections[i][field] = (input.type === 'number' || field === 'widthPct' || field === 'heightPct') ? Number(input.value || 0) : input.value;
      if (field === 'bgColor' || field === 'borderColor') updateSectionColorButton(input);
      updatePreview();
    })));
    box.querySelectorAll('[data-section-nudge]').forEach((btn)=>btn.addEventListener('click',()=>{
      const [rawIndex, axis, rawDelta] = String(btn.dataset.sectionNudge || '').split(':');
      const i = Number(rawIndex); const delta = Number(rawDelta || 0);
      if (!emailSections[i]) return;
      if (axis === 'width') emailSections[i].widthPct = Math.max(20, Math.min(100, Math.round((Number(emailSections[i].widthPct || 100) + delta) / 20) * 20));
      if (axis === 'height') emailSections[i].heightPct = Math.max(5, Math.min(40, Math.round((Number(emailSections[i].heightPct || 5) + delta) / 5) * 5));
      renderEmailSections(); updatePreview();
    }));
  ...
  function sectionFromModule(type) {
    ...
    return {
      ...
      widthPct: Math.max(20, Math.min(100, Math.round(Number(module.widthPct || 100) / 20) * 20)),
      heightPct: Math.max(5, Math.min(40, Math.round(Number(module.heightPct || 5) / 5) * 5)),
    };
  }
  ...
  function renderEmailSectionRows(position, context = {}){
    ...
    return rows.map((section)=>{
      ...
      const widthPct = Math.max(20, Math.min(100, Math.round(Number(section.widthPct || 100) / 20) * 20));
      const heightPct = Math.max(5, Math.min(40, Math.round(Number(section.heightPct || 5) / 5) * 5));
      ...
      return `<tr><td style="padding:10px 0;text-align:left;"><table role="presentation" width="${widthPct}%" cellspacing="0" cellpadding="0" border="0" style="width:${widthPct}%;max-width:100%;background:${escapeHtml(bg)};border:${borderWidth}px solid ${escapeHtml(border)};border-radius:${radius}px;"><tr><td style="padding:${padding}px;font-family:Arial,Helvetica,sans-serif;text-align:left;min-height:${minHeight}px;"><strong style="display:block;color:#111827;font-size:15px;margin-bottom:5px;">${escapeHtml(section.title || sectionLabel(section.type))}</strong><p style="margin:0;color:#4b5563;font-size:14px;line-height:1.55;">${escapeHtml(section.text || '')}</p>${button}</td></tr></table></td></tr>`;
    }).join('');
  }
  ...
  function loadEmailTemplate(id) {
    ...
    if (el('msg-layout-preset')) el('msg-layout-preset').value = d.layoutPreset || 'classic';
    if (el('msg-color')) el('msg-color').value = d.accentColor || '#111827';
    if (el('msg-star-color')) el('msg-star-color').value = d.starColor || '#f5b301';
    if (el('msg-show-top-stars')) el('msg-show-top-stars').value = String(d.showTopStars !== false);
    ...
    if (el('msg-intro')) el('msg-intro').value = d.intro || 'Hi {{ customerName }}';
    ...
    if (el('msg-product-show-stars')) el('msg-product-show-stars').value = String(d.productShowStars !== false);
    if (el('msg-product-star-position')) el('msg-product-star-position').value = d.productStarPosition || 'above_button';
    if (el('msg-product-title-weight')) el('msg-product-title-weight').value = d.productTitleWeight || '700';
  ...
  async function sendFullFakeOrderEmail() {
    const email = val('msg-test-recipient') || val('msg-test-email');
    if (!email) return showToast('Enter a test customer email first.');
    const hostError = refreshSmtpHostHint();
    if (hostError) { renderLastSendResult({ ok:false, title:'Email provider needs fixing', detail:hostError, meta:[`Recipient: ${email}`], html: buildRenderedTestEmailHtml(opts(), `test-${Date.now()}`) }); return; }
    updatePreview();
    const o = opts();
  ...
  async function searchShopifyPagesForModule() {
    const q = val('msg-module-button-url') || val('msg-module-title') || 'contact';
    const box = el('msg-module-page-results');
    if (box) box.innerHTML = '<div class="msg-help">Searching Shopify pages…</div>';
    try {
      const data = await securedFetch(`/admin/shopify-pages/search?q=${encodeURIComponent(q)}`);
      const pages = data.pages || [];
      if (!box) return;
      if (!pages.length) { box.innerHTML = '<div class="msg-help">No pages found. Type a path manually, or create the required review pages from Review link defaults.</div>'; return; }
      box.innerHTML = pages.map((page)=>`<button type="button" class="msg-page-result" data-page-url="${escapeHtml(page.url || `/pages/${page.handle}`)}"><strong>${escapeHtml(page.title || page.handle)}</strong><span>${escapeHtml(page.url || `/pages/${page.handle}`)}</span></button>`).join('');
      box.querySelectorAll('[data-page-url]').forEach((btn)=>btn.addEventListener('click',()=>{ if(el('msg-module-link-type')) el('msg-module-link-type').value='internal'; if(el('msg-module-button-url')) el('msg-module-button-url').value=btn.dataset.pageUrl || ''; box.innerHTML=''; updatePreview(); }));
    } catch (error) { if (box) box.innerHTML = `<div class="msg-help warning">Could not search pages: ${escapeHtml(error.message || 'Shopify page search failed')}</div>`; }
  }

  async function createMissingReviewPage(handle = DEFAULT_PAGE_HANDLE) {
    const clean = cleanHandle(handle);
    const box = el('msg-review-page-status');
    if (box) box.insertAdjacentHTML('beforeend', '<p class="msg-help">Creating Shopify page…</p>');
    try {
      const result = await securedFetch('/admin/storefront-pages/create', { method:'POST', body: JSON.stringify({ type: clean === 'reviews' ? 'all_reviews' : 'leave_review', handle: clean }) });
      showToast(result.message || `Created /pages/${clean}`);
      await loadReviewPageStatus();
    } catch (error) {
      showToast(error.message || 'Could not create Shopify page');
      if (box) box.insertAdjacentHTML('beforeend', `<p class="msg-help warning"><strong>Create page failed:</strong> ${escapeHtml(error.message || '')}</p>`);
    }
  }
  ...
  async function loadReviewPageStatus() {
    ...
    try {
      ...
      if (box) {
        ...
        const createBtn = status === 'missing' ? `<button type="button" class="msg-btn" data-create-review-page="${escapeHtml(handle)}">Create Shopify page</button>` : '';
        const warningNote = status === 'warning' ? '<p class="msg-help">This does not block saving templates or sending links; the button URL will still use the handle above.</p>' : '';
        box.innerHTML = `<div><strong>${pageStatusLabel(status)}: /pages/${escapeHtml(handle)}</strong><p>${escapeHtml(page?.detail || 'Could not verify this page.')}</p>${page?.url ? `<small><a href="${escapeHtml(page.url)}" target="_blank" rel="noopener">Open storefront page</a></small>` : ''}${warningNote}</div>${createBtn}`;
        box.querySelector('[data-create-review-page]')?.addEventListener('click', (event)=>createMissingReviewPage(event.currentTarget.dataset.createReviewPage));
      }
    } catch (error) {
  ...
  function currentTemplatePayload() { const o = opts(); return { name: `${val('msg-test-name','Customer')} · #${val('msg-test-order','1001')}`, customerName: val('msg-test-name','Alex'), customerEmail: val('msg-test-email','alex@example.com'), order: val('msg-test-order','1001'), mode: val('msg-test-type','order'), products: products.slice(), pageHandle: o.pageHandle, linkMode: o.linkMode, testUrl: testReviewUrl(o, val('msg-test-type','order')), createdAt: new Date().toISOString() }; }
  ...
  function applyTemplate(index) { const t = reviewTemplates[index]; if (!t) return; if (el('msg-test-name')) el('msg-test-name').value = t.customerName || 'Alex'; if (el('msg-test-email')) el('msg-test-email').value = t.customerEmail || 'alex@example.com'; if (el('msg-test-order')) el('msg-test-order').value = t.order || '1001'; if (el('msg-test-type')) el('msg-test-type').value = t.mode || 'order'; if (el('msg-page-handle') && t.pageHandle) el('msg-page-handle').value = cleanHandle(t.pageHandle); if (el('msg-link-mode') && t.linkMode) el('msg-link-mode').value = t.linkMode; products.splice(0, products.length, ...(t.products || [])); renderProductList(); updatePreview(); scheduleReviewPageStatusCheck(); showToast('Template loaded'); }
  ...
  function opts() {
    ...
    return {
      ...
      intro: val('msg-intro', 'Hi {{ customerName }}'),
      ...
      layoutPreset: val('msg-layout-preset', 'classic'),
  ...
  function topStarsEmailRow(o) {
    if (o.showTopStars === false) return '';
    return `<tr><td align="center" style="padding:0 0 12px 0;">${productStars(o.starColor || '#f5b301')}</td></tr>`;
  }
  ...
  function buildFlowEmailHtml(o) {
    ...
    const productButtons = `<tr><td style="padding:18px 0 0 0;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">{% for line_item in order.lineItems %}${productRowEmailHtml({ o, flow: true, url: flowReviewUrl(o, 'product', '&product_id={{ line_item.product.id }}&variant_id={{ line_item.variant.id }}&product_title={{ line_item.title | url_encode }}') })}{% endfor %}</table></td></tr>`;
    const topStars = topStarsEmailRow(o);
    const links = o.linkMode === 'order' ? orderButton + topStars : o.linkMode === 'products' ? topStars + productButtons : orderButton + topStars + productButtons;
  ...
  function buildRenderedTestEmailHtml(o, tokenOverride = '') {
    ...
    const productRows = `<tr><td style="padding:18px 0 0 0;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">${safeProducts.map((product) => {
      ...
      return productRowEmailHtml({ o, product, url });
    }).join('')}</table></td></tr>`;
    const topStars = topStarsEmailRow(o);
    const links = o.linkMode === 'order' ? orderButton + topStars : o.linkMode === 'products' ? topStars + productRows : orderButton + topStars + productRows;
    const intro = o.intro.replace(/\{\{[^}]+\}\}/g, escapeHtml(val('msg-test-name', 'Alex')));
    const testOpts = { ...o, intro, footerLabel: `Sent by ${friendlyShopName()}.` };
  ...
  async function saveProviderProfile() {
    ...
    const hostError = smtpHostClientError(payload.smtpHost);
  ...
  function bind() {
    ...
    el('msg-module-search-page')?.addEventListener('click', () => searchShopifyPagesForModule());
    ...
    ['msg-color','msg-bg-color','msg-card-color','msg-star-color','msg-module-bg','msg-module-border'].forEach((id)=>{ const input = el(id); if (input) input.addEventListener('input', ()=>refreshColorButton(input)); });
    refreshAllColorButtons();
    ...
    el('msg-smtp-provider')?.addEventListener('change', () => { applyProviderPreset(true); refreshSmtpHostHint(); });
    el('msg-apply-provider-preset')?.addEventListener('click', () => { applyProviderPreset(true); refreshSmtpHostHint(); showToast('Provider host applied'); });
    el('msg-smtp-host')?.addEventListener('input', refreshSmtpHostHint);
