...
(function () {
  ...
  function moduleFormPayload() {
    ...
    return {
      ...
      buttonUrl: normaliseButtonUrl(val('msg-module-button-url')),
  ...
  function renderAnalyticsList() {
    ...
    if (currentAnalyticsList === 'recipients') {
      ...
      box.innerHTML = rows.map((row) => `<div class="msg-analytics-row recipient"><div><strong>${escapeHtml(row.email || 'No email')}</strong>${row.isTest ? '<span class="msg-test-pill">Test</span>' : ''}<br><small>${escapeHtml(row.campaign || 'review_request')} · Order ${escapeHtml(row.orderId || '—')}</small>${sentDetailsHtml(row)}</div><div><small>Sent</small><br>${row.sentAt ? new Date(row.sentAt).toLocaleString() : '—'}</div><div><small>Opened</small><br>${row.openedAt ? new Date(row.openedAt).toLocaleString() : 'Not opened'}</div><div><small>Clicked / Reviewed</small><br>${row.clickedAt ? new Date(row.clickedAt).toLocaleDateString() : '—'}${row.reviewedAt ? ` / ${new Date(row.reviewedAt).toLocaleDateString()}` : ''}</div><div><button type="button" class="msg-reminder-btn" data-remind-email="${escapeHtml(row.email || '')}" data-remind-order="${escapeHtml(row.orderId || '')}" data-remind-item="${escapeHtml(row.itemId || '')}" ${row.openedAt || !row.email ? 'disabled' : ''}>Reminder</button></div></div>`).join('');
      box.querySelectorAll('[data-remind-email]').forEach((btn)=>btn.addEventListener('click',()=>sendReminder(btn.dataset.remindEmail, btn.dataset.remindOrder, btn.dataset.remindItem).catch((error)=>showToast(error.message || 'Could not send reminder'))));
      return;
    }
  ...
  async function sendReminder(email, orderId, itemId) { if (!email) return; await securedFetch('/admin/campaign-reminder', { method: 'POST', body: JSON.stringify({ email, orderId, itemId, campaign: 'manual_review_reminder' }) }); showToast('Reminder sent'); await loadAnalytics(); }
