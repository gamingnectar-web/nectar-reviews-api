  ...
  async function loadReviewPageStatus() {
    ...
    try {
      ...
      if (box) {
        ...
        box.innerHTML = `<div><strong>${pageStatusLabel(status)}: /pages/${escapeHtml(handle)}</strong><p>${escapeHtml(page?.detail || 'Could not verify this page.')}</p>${page?.url ? `<small><a href="${escapeHtml(page.url)}" target="_blank" rel="noopener">Open storefront page</a></small>` : ''}</div>${createBtn}`;
