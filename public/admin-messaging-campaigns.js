...
(function () {
  ...
  async function loadReviewPageStatus() {
    ...
    box.innerHTML = `<span class="msg-page-status-dot">…</span><div><strong>Checking /pages/${escapeHtml(handle)}…</strong><p>Verifying the customer review page before emails are sent.</p></div><button type="button" id="msg-refresh-page-status" class="msg-btn secondary">Check page</button>`;
    try {
      const data = await api(`/admin/storefront-page-checks?reviewPageHandle=${encodeURIComponent(handle)}&allReviewsPageHandle=reviews`);
      const page = (data.pages || []).find((item) => item.handle === handle) || data.pages?.[0];
      const status = page?.status || 'warning';
      box.dataset.status = status;
      box.innerHTML = `<span class="msg-page-status-dot">${pageStatusIcon(status)}</span><div><strong>${pageStatusLabel(status)}: /pages/${escapeHtml(handle)}</strong><p>${escapeHtml(page?.detail || 'Could not verify this page.')}</p>${page?.url ? `<small><a href="${escapeHtml(page.url)}" target="_blank" rel="noopener">Open storefront page</a></small>` : ''}</div><button type="button" id="msg-refresh-page-status" class="msg-btn secondary">Check page</button>`;
    } catch (error) {
      box.dataset.status = 'warning';
      box.innerHTML = `<span class="msg-page-status-dot">⚠</span><div><strong>Could not verify review page</strong><p>${escapeHtml(error.message || 'Check Render logs and Shopify connection.')}</p></div><button type="button" id="msg-refresh-page-status" class="msg-btn secondary">Check page</button>`;
    }
  ...
  function bind() {
    ...
    el('msg-refresh-page-status')?.addEventListener('click', () => loadReviewPageStatus());
    el('msg-page-handle')?.addEventListener('change', () => loadReviewPageStatus());
