...
/* Keep dashboard overview focused. Discount details now live under their own nav group. */
.nr-dashboard-strip {
...
function findLineStartBefore(index) {
  const lineStart = html.lastIndexOf('\n', index);
  return lineStart === -1 ? 0 : lineStart + 1;
}

function findManageStart() {
  const direct = html.search(/<[^>]*(section-label|section-title|nav-label|nav-heading)[^>]*>\s*MANAGE\s*<\/[^>]+>/i);
  if (direct !== -1) return findLineStartBefore(direct);

  const dashboard = html.search(/Dashboard/i);
  if (dashboard === -1) return -1;

  const before = html.lastIndexOf('<div', dashboard);
  return before === -1 ? findLineStartBefore(dashboard) : findLineStartBefore(before);
}

function findConfigStart(fromIndex) {
  const direct = html.slice(fromIndex).search(/<[^>]*(section-label|section-title|nav-label|nav-heading)[^>]*>\s*CONFIGURATION\s*<\/[^>]+>/i);
  if (direct !== -1) return fromIndex + direct;

  const rules = html.indexOf('Rules & Settings', fromIndex);
  if (rules === -1) return -1;

  const configWord = html.lastIndexOf('CONFIGURATION', rules);
  if (configWord !== -1 && configWord > fromIndex) {
    const tagStart = html.lastIndexOf('<', configWord);
    return tagStart === -1 ? findLineStartBefore(configWord) : findLineStartBefore(tagStart);
  }

  const rulesButton = html.lastIndexOf('<button', rules);
  return rulesButton === -1 ? findLineStartBefore(rules) : findLineStartBefore(rulesButton);
}

const manageStart = findManageStart();
const configStart = findConfigStart(manageStart);

if (manageStart === -1 || configStart === -1 || configStart <= manageStart) {
  console.log('Could not safely find sidebar area.');
  console.log('Here are matching lines from admin.html:');
  html.split('\n').forEach((line, i) => {
    if (/MANAGE|CONFIGURATION|Dashboard|Review Manager|Discount Rewards|Rules & Settings/i.test(line)) {
...
console.log('Done. Cleaned sidebar navigation and set Dashboard Overview as default.');
