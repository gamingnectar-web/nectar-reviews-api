...
/**
 * Hard reset admin.js:
 * Keep the original app code up to the real base window.load();
 * Remove all stacked nav/sidebar patches after it.
 */
const loadMarker = 'window.load();';
const cutAt = js.lastIndexOf(loadMarker);

if (cutAt === -1) {
  throw new Error('Could not find base window.load(); in admin.js');
}

js = js.slice(0, cutAt + loadMarker.length) + '\n';
...
if (!html.includes('HARD RESET fast top tabs + contextual side tabs')) {
...
(function () {
    ...
    '<div id="reward-manual" class="nr-discount-subview">',
      '<div class="nr-manual-code-grid">',
        '<div class="panel">',
          ...
          '<button class="post-btn" type="button" onclick="window.createManualRewardCode()">Create Manual Code</button>',
        '</div>',
        '<div class="panel">',
          ...
          '<div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:14px;">',
            '<button class="secondary-btn" type="button" onclick="window.copyManualRewardCode()">Copy Code</button>',
            '<button class="secondary-btn" type="button" onclick="window.copyManualRewardEmail()">Copy Email Copy</button>',
          '</div>',
        '</div>',
      '</div>',
    '</div>',
      ...
      '<div class="nr-email-template-grid">',
        '<div class="panel">',
          ...
          '<div class="nr-variable-row">',
            '<button class="nr-variable-chip" type="button" onclick="window.insertRewardVariable(\\'{{ discount_code }}\\')">{{ discount_code }}</button>',
            '<button class="nr-variable-chip" type="button" onclick="window.insertRewardVariable(\\'{{ expiry_days }}\\')">{{ expiry_days }}</button>',
            '<button class="nr-variable-chip" type="button" onclick="window.insertRewardVariable(\\'{{ percentage }}\\')">{{ percentage }}</button>',
            '<button class="nr-variable-chip" type="button" onclick="window.insertRewardVariable(\\'{{ customer_email }}\\')">{{ customer_email }}</button>',
          '</div>',
          ...
          '</div>',
          '<button class="post-btn" type="button" onclick="window.saveRewardSettings()">Save Email Template</button>',
        '</div>',
        '<div class="panel">',
          '<h3 style="margin-top:0;">Live Preview</h3>',
          '<div id="reward-email-preview" class="nr-email-preview"></div>',
          '<div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:14px;">',
            '<button class="secondary-btn" type="button" onclick="window.renderRewardEmailPreview()">Refresh Preview</button>',
            '<button class="secondary-btn" type="button" onclick="window.copyRewardEmailPreview()">Copy Preview</button>',
          '</div>',
        '</div>',
      '</div>',
    '</div>'
  ...
  function ensureTopTabs() {
    document.querySelectorAll('.nr-primary-tabs').forEach(el => el.remove());
  ...
  function renderSidebar() {
    const group = findMainSidebarGroup();
    if (!group) return;
  ...
  function showDiscountSubView(sub) {
    ...
    if (sub === 'codes' && !state.rewardCodesLoaded) {
      ...
      later(() => window.loadRewardCodes());
    }

    if (sub === 'email') {
      later(() => window.renderRewardEmailPreview());
    }
  }
  ...
  function select(module, sub) {
    ...
    if (state.module === 'discounts') {
      ...
      if (!state.rewardSettingsLoaded) {
        ...
        later(() => window.loadRewardSettings());
      }
    }
  ...
  document.addEventListener('click', function(event) {
    const button = event.target.closest('[data-nr-module][data-nr-sub]');
    if (!button) return;
    ...
    select(button.dataset.nrModule, button.dataset.nrSub);
  }, true);
...
console.log('Done. admin.js has been hard-reset to one fast nav controller.');
